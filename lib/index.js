...
var path = require( 'path' ),
	adapterPath = path.resolve( __dirname, './adapter.js' ),
...
module.exports = {
	...
	attach: function() {
		...
		this.plugins.addFiles( [ adapterPath, sinonPath, sinonIEPath ] );

		function build( data ) {
			data.parts.push(
				'<head>' +
				'<script src="' + path.join( '/plugins', adapterPath ).split( path.sep ).join( '/' ) + '"></script>' +
